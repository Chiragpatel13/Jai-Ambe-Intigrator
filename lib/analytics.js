...
export async function getDetailedAnalytics() {
  ...
  try {
    ...
    const dailyStats30 = await Promise.all(last30Days.map((date) => readDailyFromFirestore(date)));
    ...
    const dailyStats30 = last30Days.map((date) => mockAnalytics.daily[date] || emptyDaily(date));
